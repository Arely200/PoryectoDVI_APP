...
const ALTURA_ZONA_PLATO = height * 0.42;
...
export default function JuegoArmarPlato({ route, navigation }) {
  ...
  async function confirmarPlato() {
    ...
    const colocadosChatarra = items.filter((item, i) => colocados[i] && !item.saludable).length;
    const puntaje = Math.max(0, colocadosSaludables - colocadosChatarra);
